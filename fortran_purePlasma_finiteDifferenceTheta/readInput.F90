  module readInput
    ...
    contains
      ...
      subroutine readNamelistInput(filename)
        ...
        namelist / parameters / epsil, nuPrime, geometryToUse,  &
             useIterativeSolver, multipleOfNuPrimeToAddOnDiagonalOfPreconditioner, &
             thetaDerivativeScheme, whichParallelSolverToFactorPreconditioner, &
        ...
        if (didFileAccessWork /= 0) then
           ...
           if (didFileAccessWork /= 0) then
              print *,"Proc ",myRank,": Error!  I was able to open the file ", filename, " but not read data from it."
              stop
           end if
           if (masterProc) then
