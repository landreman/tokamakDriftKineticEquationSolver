function [soln, didItConverge, residual] ...
    = solveLinearSystem(matrix, rhs, preconditionerHandle, ...
...
% If figureNum = 0, the current figure will be used.
% If figureNum < 0, no figure will be plotted.
...
if ~tryIterativeSolvers
    ...
    for col=1:numCols
        ...
        while keepTrying
            ...
            residualVector = matrix*soln0 - thisRHS;
            L1_normalization = sum(abs(thisRHS));
            L2_normalization = sqrt(sum(thisRHS.*thisRHS));
            L_inf_normalization = max(abs(thisRHS));
