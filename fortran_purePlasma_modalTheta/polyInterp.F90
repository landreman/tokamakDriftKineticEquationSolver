...
#include <finclude/petscsysdef.h>
    ...
    subroutine makePolynomialInterpolationMatrix(xk, x, alpxk, alpx, matrix)
      ! Inputs:
      !   xk = grid points on which the function is known.
      !   alpxk = weight function evaluated on the xk grid.
      !   x = grid onto which we would like to interpolate.
      !   alpx = weight function evaluated on the x grid.
      !
      ! Outputs:
      ...
      PetscScalar, dimension(:), intent(in) :: xk, x, alpxk, alpx
      integer N, M, i
      PetscScalar, intent(out) :: matrix(:,:)
      PetscScalar, dimension(:,:), allocatable :: xkMatrixified, D, xMatrixified
      PetscScalar, dimension(:), allocatable :: w

      N = size(xk);
      M = size(x);
      if (size(alpxk) /= N) then
        print *,"Error! size(alpxk) /= size(xk)"
        stop
      end if

      if (size(alpx) /= M) then
        print *,"Error! size(alpx) /= size(x)"
        stop
      end if
