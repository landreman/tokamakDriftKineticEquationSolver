function [thetaSurfs, BSurfs, BDotGradThetaSurfs, qSurfs, as, R0, B0] = getGeometryFromEFITForSeveralFluxSurfaces(filename, desiredPsiNs, topCropZ, bottomCropZ, plotStuff)
...
for i=1:N
    ...
    % Sometimes the contour is calculated with increasing theta, and
    % sometimes it is calculated with decreasing theta. Assume that
    % whichever occurs for most of the points is correct.
    if nnz(thetaIncreasing)/numel(thetaIncreasing) > 0.5
    ...
    if thetaIncreasing(1) && thetaIncreasing(end)
        % The troublesome wedge is split across the ends of the vector.
        indexToIgnore = find(~thetaIncreasing,1,'first')-1;
    else
        % The troublesome wedge occurs in the middle of the vector.
        indexToIgnore = find(thetaIncreasing,1,'last');
    end
    thetaIncreasing(indexToIgnore) = false;
